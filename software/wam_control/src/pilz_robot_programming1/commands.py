...
_DEFAULT_TARGET_LINK = "wam/racquet_hitpoint_link"
_DEFAULT_GRIPPER_PLANNING_GROUP = "gripper"
_DEFAULT_BASE_LINK = "wam/base_link"
...
class BaseCmd(_AbstractCmd):
    ...
    def _cmd_to_request(self, robot):
        ...
        self._start_joint_states = robot.get_current_joint_states(
            planning_group=self._planning_group)
        self._start_pose = robot.get_current_pose(
            target_link=self._target_link, base=self._reference_frame)
        ...
        req.max_acceleration_scaling_factor = self._acc_scale * \
            self._calc_acc_scale(robot._speed_override)
        req.allowed_planning_time = 1.0

        # Set an empty diff as start_state => the current state is used by the planner
        req.start_state.is_diff = True
        # req.start_state = robot._robot_commander.get_current_state()
    ...
    def _check_header_time(self):
        if self._goal.header.stamp != rospy.Time(0, 0):
            raise ValueError(
                "Given goal has unsupported time for future execution.")
...
class Ptp(BaseCmd):
    ...
    def __init__(self, vel_scale=_DEFAULT_JOINT_VEL_SCALE, acc_scale=None, *args, **kwargs):
        acc_scale_final = acc_scale if acc_scale is not None else Ptp._calc_acc_scale(
            vel_scale)
        super(Ptp, self).__init__(vel_scale=vel_scale,
                                  acc_scale=acc_scale_final, *args, **kwargs)
...
class Lin(BaseCmd):
    ...
    def __init__(self, vel_scale=_DEFAULT_CARTESIAN_VEL_SCALE, acc_scale=None, *args, **kwargs):
        ...
        super(Lin, self).__init__(vel_scale=vel_scale,
                                  acc_scale=acc_scale_final, *args, **kwargs)
...
class Circ(BaseCmd):
    ...
    def __init__(self, interim=None, center=None, vel_scale=_DEFAULT_CARTESIAN_VEL_SCALE, acc_scale=None,
        ...
        super(Circ, self).__init__(vel_scale=vel_scale,
                                   acc_scale=acc_scale_final, *args, **kwargs)
    ...
    def _cmd_to_request(self, robot):
        ...
        if self._reference_frame:
            path_point = self._to_robot_reference(
                self._reference_frame, path_point)
