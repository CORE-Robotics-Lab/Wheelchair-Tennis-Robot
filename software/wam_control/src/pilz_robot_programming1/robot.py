    ...
    * :py:class:`.Gripper`
    ...
    def get_current_pose_stamped(self, target_link=_DEFAULT_TARGET_LINK, base=_DEFAULT_BASE_LINK):
        ...
        try:
            ...
            current_pose = self.tf_buffer_.transform(
                zero_pose, base, rospy.Duration(5, 0))
            return current_pose
    ...
    def move(self, cmd):
        ...
        if not self._move_lock.acquire(False):
            raise RobotMoveAlreadyRunningError(
                "Parallel calls to move are note allowed.")
    ...
    def is_brake_test_required(self):
        ...
        try:
            rospy.wait_for_service(
                self._BRAKE_TEST_REQUIRED_SRV, self._SERVICE_WAIT_TIMEOUT_S)
            ...
            if resp.result.value == IsBrakeTestRequiredResult.REQUIRED:
                ...
                rospy.logerr(
                    "Failure during call of braketest required service: BrakeTestRequirementStatus UNKNOWN")
                raise rospy.ROSException(
                    "Could not determine if braketest is required.")
        except rospy.ROSException as e:
            rospy.logerr(
                "Failure during call of braketest required service: {0}".format(e))
            raise e
    ...
    def execute_brake_test(self):
        ...
        try:
            ...
            rospy.logerr(
                "Failure during call of braketest execute service: {0}".format(e))
            raise e
    ...
    def _get_execute_brake_test_service(self):
        try:
            rospy.wait_for_service(
                self._BRAKE_TEST_EXECUTE_SRV, self._SERVICE_WAIT_TIMEOUT_S)
        except rospy.ROSException as e:
            rospy.logerr(
        ...
        while continue_execution_of_cmd:
            ...
            if ((self._move_ctrl_sm.state == _MoveControlState.NO_REQUEST and first_iteration_flag) or
                ...
                if execution_result == Robot._STOPPED:
                    ...
                    if self._move_ctrl_sm.state == _MoveControlState.PAUSE_REQUESTED \
                        ...
                        if isinstance(cmd, Sequence):
                            rospy.logerr(
                                "Pause not implemented for sequence yet")
                            raise RobotMoveFailed(
                                "Pause not implemented for sequence yet")
                    # external stop
                    ...
                    # normal stop
                        ...
                        raise RobotMoveFailed(
                            "Execution of move command is stopped")
                # motion succeeded
                ...
                # motion failed
                    ...
                    raise RobotMoveFailed(
                        "Failure during execution of: " + str(cmd))
    ...
    def _claim_single_instance(self):
        ...
        if self._check_single_instance():
            ...
            raise RobotMultiInstancesError(
                "Only one instance of Robot class can be created!")
        ...
        if rospy.has_param(self._INSTANCE_PARAM):
            ...
            if psutil.pid_exists(pid):
                ...
                if process.create_time() == create_time:
                    rospy.logerr(
                        "An instance of Robot class already exists (pid=" + str(pid) + ").")
                    return False
    ...
    def _establish_connections(self):
        ...
        self._sequence_client = SimpleActionClient(
            self._SEQUENCE_TOPIC, MoveGroupSequenceAction)
        rospy.loginfo("Waiting for connection to action server " +
                      self._SEQUENCE_TOPIC + "...")
        self._sequence_client.wait_for_server()
        rospy.logdebug("Connection to action server " +
                       self._SEQUENCE_TOPIC + " established.")
        ...
        rospy.loginfo("Waiting for connection to service " +
                      self._GET_SPEED_OVERRIDE_SRV + "...")
        rospy.wait_for_service(self._GET_SPEED_OVERRIDE_SRV,
                               self._SERVICE_WAIT_TIMEOUT_S)
        rospy.loginfo("Connection to service " +
                      self._GET_SPEED_OVERRIDE_SRV + " established")
        self._get_speed_override_srv = rospy.ServiceProxy(
            self._GET_SPEED_OVERRIDE_SRV, GetSpeedOverride)
    ...
    def _release(self):
        ...
        try:
            ...
            rospy.logdebug(
                "Services do not exists yet or have already been shutdown.")
        # do not delete pid parameter if it has not been set or overwritten
        if self._single_instance_flag:
            rospy.logdebug(
                "Delete single instance parameter from parameter server.")
            try:
