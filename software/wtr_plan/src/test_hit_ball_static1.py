...
class MoveGroupInterface(object):
    ...
    def __init__(self):
        ...
        move_group.set_planning_time(.2) #c
        # move_group.set_planner_id("RRTstarkConfigDefault")

        move_group.set_max_acceleration_scaling_factor(0.8) #c
        move_group.set_max_velocity_scaling_factor(0.8) #c
    ...
    def move_pose(self, pose):
      ...
      # self.move_group.set_goal_tolerance(0.07)
...
def make_linear_swing(point):
  ...
  # base_pose.orientation.x = 0.706011
  # base_pose.orientation.y = 0.045011
  # base_pose.orientation.z = -0.706011
  # base_pose.orientation.w = -0.045011
...
def observation_callback(odometry_msg):
    ...
    # rospy.loginfo(relative_ball_ps)

    # test code for linear swing
    # waypoints = make_linear_swing(ball_p)
    ...
    if ball_reset_cnt == 15:
      ...
      if test_goal.position.z > 0.5 and test_goal.position.z < 2.0:
        # waypoints = make_linear_swing(ball_ps.point)
        # interface.move_pose(waypoints[0])
    ...
    # try:
    #     response = swing_service(ball_position=relative_ball_ps)
...
def spawn_random_ball(event):
    ...
    # Spawn Random Ball
    pose = Pose(position=Point(15, -1.1, 3.5), orientation=Quaternion(w=1))
    ...
    # test_goal.orientation.z = -0.706011
    # test_goal.orientation.w = -0.045011
    # test_goal.position.x = 0.149011
    # test_goal.position.y = -1.266011
    # test_goal.position.z = 1.293011
    # interface.move_pose(test_goal) 
...
def main():
    ...
    if SIM:
        ...
        publish_state_timer = rospy.Timer(rospy.Duration(8.0), spawn_random_ball)
    ...
    # test_goal.position.y = -1.266011
    # test_goal.position.z = 1.293011
    # # interface.set_goal_tolerance(0.5)
    # interface.move_pose(test_goal) 
